...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Auto Industrial SCARA Debugger (Modular)")
        
        self.brain = RobotBrain(L1, L2, BASE_L, BASE_R)
        self.brain.update_callback = self.update_info_text 
    ...
    def setup_ui(self):
        ...
        ttk.Button(ui, text="TRIPLE TAP DELETE", command=self.manual_delete).grid(row=0, column=1, padx=5)
        ttk.Button(ui, text="ADD OBJ", command=self.add_shape).grid(row=0, column=2, padx=5)
        ttk.Button(ui, text="RESET", command=self.reset).grid(row=0, column=3, padx=5)

        self.canvas.bind("<Button-1>", self.handle_press)
        self.canvas.bind("<B1-Motion>", self.handle_motion)
        self.canvas.bind("<ButtonRelease-1>", lambda e: setattr(self.brain, 'is_down', False))

    def update_info_text(self, x, y):
        self.info_label.config(text=f"COORD -> X: {x:6.1f} mm | Y: {y:6.1f} mm | REACH: {np.hypot(x,y):.1f}/445")
    ...
    def handle_press(self, event):
        ...
        for s in self.shapes:
            if np.hypot(s.x - mx, s.y - my) < 25:
                hit = s; break
        self.brain.selected_shape = hit

        if self.brain.compute_all_ik(mx, my):
            threading.Thread(target=self.brain.move_to, args=(mx, my), daemon=True).start()
        
        if hit and self.click_count == 3: self.manual_delete()
    ...
    def draw_loop(self):
        ...
        cx_l, cy_l = 400 + BASE_L[0] * self.scale, 700 - BASE_L[1] * self.scale
        self.canvas.create_oval(cx_l - rmax, cy_l - rmax, cx_l + rmax, cy_l + rmax, 
                                 outline="#333", dash=(4, 4))
        
        cx_r, cy_r = 400 + BASE_R[0] * self.scale, 700 - BASE_R[1] * self.scale
        self.canvas.create_oval(cx_r - rmax, cy_r - rmax, cx_r + rmax, cy_r + rmax, 
                                 outline="#333", dash=(4, 4))

        
        ex = 400 + (-SCR_W/2)*self.scale
        ey = 700 - (SCR_Y+SCR_H)*self.scale
        self.canvas.create_rectangle(ex, ey, ex+SCR_W*self.scale, ey+SCR_H*self.scale, 
        ...
        size = 10 if self.brain.is_down else 7
        self.canvas.create_oval(px-size, py-size, px+size, py+size, fill=color, outline="black", width=2)
